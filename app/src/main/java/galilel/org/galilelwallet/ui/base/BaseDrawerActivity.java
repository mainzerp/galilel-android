...
import galilel.org.galilelwallet.ui.contacts_activity.ContactsActivity;
import galilel.org.galilelwallet.ui.donate.DonateActivity;
import galilel.org.galilelwallet.ui.settings_activity.SettingsActivity;
import galilel.org.galilelwallet.ui.wallet_activity.WalletActivity;

import static galilel.org.galilelwallet.module.GalilelContext.OUT_OF_SYNC_TIME;
...
public class BaseDrawerActivity extends GalilelActivity implements NavigationView.OnNavigationItemSelectedListener {
    ...
    private void checkState(){
        ...
        long lastBlockTime = galilelApplication.getAppConf().getLastBestChainBlockTime();
        if (lastBlockTime+OUT_OF_SYNC_TIME>now){
            // check if i'm syncing or i'm synched
            long peerHeight = galilelModule.getConnectedPeerHeight();
            if (peerHeight!=-1){
                if (galilelModule.getChainHeight()+10>peerHeight) {
            ...
            if (galilelModule.isAnyPeerConnected()) {
                long peerHeight = galilelModule.getConnectedPeerHeight();
                if (peerHeight!=-1){
                    if (galilelModule.getChainHeight()+10>peerHeight) {
                        blockchainState = BlockchainState.SYNC;
