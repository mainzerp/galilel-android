...
import org.galilelj.crypto.KeyCrypter;
...
import global.galilel.DefaultCoinSelector;
...
public class GalilelModuleImp implements GalilelModule {
    ...
    private BigDecimal galiInUsdHardcoded = new BigDecimal("1.5");

    // OS dependent helper.
    private BackupHelper backupHelper;

    public GalilelModuleImp(ContextWrapper contextWrapper, WalletConfiguration walletConfiguration,ContactsStoreDao contactsStore,RateDbDao rateDb,BackupHelper backupHelper) {
    ...
    // todo: clean this.
    //public void setGalileltrumPeergroup(GalileltrumPeergroup peergroup){
    ...
    public BigDecimal getAvailableBalanceLocale() {
        return galiInUsdHardcoded.multiply(new BigDecimal(availableBalance));
    }
    ...
    public Transaction completeTxWithCustomFee(Transaction transaction,Coin fee) throws InsufficientMoneyException{
        ...
        walletManager.completeSend(sendRequest);
        // if the fee is different to the custom fee and the tx size is lower than 1kb (1000 bytes in galilel core)
   ...
   public boolean isSyncWithNode() throws NoPeerConnectedException {
    ...
    @Override
    public List<GalilelRate> listRates() {
        return rateDb.list();
    }
