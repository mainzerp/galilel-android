...
import org.galilelj.core.CoinDefinition;
...
import galileltrum.exceptions.InvalidPeerVersion;
import galileltrum.listeners.AddressListener;
import galileltrum.listeners.PeerDataListener;
import galileltrum.listeners.PeerListener;
import galileltrum.messages.VersionMsg;
...
/**
 * Class in charge of manage the connection with galileltrum servers.
 *
...
public class GalileltrumPeergroup implements PeerListener, PeerDataListener {

    private static final Logger log = LoggerFactory.getLogger(GalileltrumPeergroup.class);
    ...
    /** Trusted peer */
    private GalileltrumPeer trustedPeer;
    // Currently active peers.
    private final CopyOnWriteArrayList<GalileltrumPeer> peers;
    // Currently connecting peers.
    private final CopyOnWriteArrayList<GalileltrumPeer> pendingPeers;
    ...
    public GalileltrumPeergroup(NetworkConf networkConf, WalletManager walletManager, AddressStore addressStore) throws IOException {
    ...
    public GalileltrumPeergroup(NetworkConf networkConf) throws IOException {
        this.peers = new CopyOnWriteArrayList<>();
        this.pendingPeers = new CopyOnWriteArrayList<>();
        this.networkConf = networkConf;
        this.ioManager = new IoManager(1,1);
    ...
    public synchronized void start() throws InterruptedException, ConnectionFailureException {
        try {
            log.info("Starting GalileltrumPeergroup");
            isActive = true;
            // todo: first part discovery..
            /*
            * Connect to the trusted node and get servers from it.
            */
            trustedPeer = new GalileltrumPeer(networkConf.getTrustedServer(), ioManager,versionMsg);
            ...
            isRunning = false;
            isActive = false;
            log.error("GalileltrumPeerGroup start",e);
            throw e;
        }
    }
    ...
    @Override
    public void onConnected(GalileltrumPeer galileltrumPeer) {
        try {
            if (galileltrumPeer == trustedPeer) {
                ...
                for (PeerListener peerConnectionListener : peerConnectionListeners) {
                    peerConnectionListener.onConnected(galileltrumPeer);
                }
                ...
                for (InetSocketAddress inetSocketAddress : networkConf.getNetworkServers()) {
                    GalileltrumPeerData peerData = new GalileltrumPeerData(inetSocketAddress.getHostName(),inetSocketAddress.getPort(),0);
                    GalileltrumPeer peer = new GalileltrumPeer(peerData,ioManager,versionMsg);
                ...
                log.info("Non trusted peer connected, "+galileltrumPeer.getPeerData());
                pendingPeers.remove(galileltrumPeer);
                peers.add(galileltrumPeer);
            }
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    @Override
    public void onDisconnected(GalileltrumPeer galileltrumPeer) {

    }

    @Override
    public void onExceptionCaught(GalileltrumPeer galileltrumPeer, Exception e) {
    ...
    @Override
    public void onSubscribedAddressChange(GalileltrumPeer galileltrumPeer, String address, String status) {
        try {
            ...
            if (statusDb.getStatus()==null || !status.equals(statusDb.getStatus())){
                ...
                for (GalileltrumPeer peer : peers) {
    ...
    @Override
    public void onListUnpent(GalileltrumPeer galileltrumPeer,String address, List<Unspent> unspents) {
    ...
    @Override
    public void onBalanceReceive(GalileltrumPeer galileltrumPeer, String address, long confirmed, long unconfirmed) {
        try {
            if (galileltrumPeer == trustedPeer) {
    ...
    @Override
    public void onGetHistory(GalileltrumPeer galileltrumPeer, StatusHistory statusHistory) {
        try {
